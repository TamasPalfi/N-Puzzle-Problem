 ...
 * used in this class to search is DFS.
 */
...
public class UninformedSearch extends Node {
	...
	//constructor method
	public UninformedSearch(Node gameState){
		super(gameState.getState());
		this.gameBoard = gameState;
	}
	
	//method to carry out the DFS search
	public Stack<Node> DFS(Node gameState) throws CloneNotSupportedException{
		//stack of gameBoards or "frontier"
		Stack<Node> gameStates = new Stack<Node>();
		...
		while(!gameStates.isEmpty()){
			...
			Node stateToCheck = gameStates.pop();
			...
			for(int z = 0; z < possibleStates.size(); z++){
				if(!gameStates.contains(possibleStates.get(z)) && !explored.contains(possibleStates.get(z))){
					gameStates.push(possibleStates.get(z));
					totalFrontierNodes++;	
				}
			}
		}
